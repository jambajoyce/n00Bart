...
public class BartActivity extends Activity {

	BartData[] cityDataList; 
	BartListAdapter mAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		cityDataList = new BartData[citiesList.length];
		...
		for (int i = 0; i < citiesList.length; i++) {
			BartData cd = new BartData(citiesList[i],"Loading...","","",i);
			cityDataList[i] = cd;
		}

		//create an instance of our customized array adapter and attach it to the listview
		ListView listView = (ListView) findViewById(R.id.bartList);
		mAdapter = new BartListAdapter(this, cityDataList);          
		listView.setAdapter(mAdapter);


		//for each city, kicks off async request to API
		for (BartData city : cityDataList) {
    ...
    private class BartCheckTask extends AsyncTask<String, Void, String> {
        ...
        public String doInBackground(String... city)
        {
           if (city != null) {
        	   ...
        	   # http://api.bart.gov/api/sched.aspx?cmd=fare&orig=12th&dest=embr&date=today
